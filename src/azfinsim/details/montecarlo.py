...
def mc_simulation(fx1, sigma1, drift, v, ro, maturity, t_steps, trials):
    ...
    ndt = np.zeros(
        (t_steps + 1, trials), np.float64
    )  # place holder array for time step counter
    ndt[0] = 0

    fx_simulation = np.zeros(
        (t_steps + 1, trials), np.float64
    )  # place holder array for simulated FX paths
    stoh_vol = np.zeros(
        (t_steps + 1, trials), np.float64
    )  # place holder array for simulated volatilities
    ...
    for t in range(1, t_steps + 1):
        ...
        random_num_2 = np.random.standard_normal(
            trials
        )  # drawing random numbers for stochastic volatility process
        random_num_1 = np.random.standard_normal(
            trials
        )  # drawing random numbers for FX process

        stoh_vol[t] = stoh_vol[t - 1] * np.exp(
            (-0.5 * v**2) * dt
            + v
            * (ro * random_num_1 + np.sqrt(1 - ro**2) * random_num_2)
            * np.sqrt(dt)
        )  # stochastic volatility process

        # print(stoh_vol[t,0])
        fx_simulation[t] = fx_simulation[t - 1] * np.exp(
            (drift - 0.5 * stoh_vol[t] ** 2) * dt
            + stoh_vol[t] * random_num_1 * np.sqrt(dt)
        )
...
def price_option(inputs):
    ...
    # calibration value: 0.0808844481978   Vega01 value: 0.081697724461
    ro = inputs["ro"]
    # calibration value: 0.000038413221829   Vega01 value: 0.0000387714624899
    v = inputs["v"]
    #   calibration value: 0.00154807378604    Vega01 value: 0.00153409902822
    ...
    Simulation = mc_simulation(
        fx1, sigma1, drift, v, ro, maturity, t_steps, trials
    )  # calling the MC function
    # print(Simulation)
    ...
    simFx = pd.DataFrame(
        sim, columns=workingDays
    )  # creating table with FX paths in rows and dt as column names
    ...
    for i in range(trials):
        cashSetAm[i] = (
            warrantsNo
            * notionalPerWarr
            * max(0, (settlementRate[i] / strike) - 1)
            * (1 / settlementRate[i])
        )
    ...
    # netSettlement netSettlement[i] = (cashSetAm[i] - warrantsPrice) * np.exp(-drift * delta.days / 365) =
    #   Cash Settlement(t0) - Warrant Price(t0)
